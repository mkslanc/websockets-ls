...
const servers = [
    {
        args: ['node', [require.resolve('svelte-language-server/bin/server.js'), "--stdio"]],
        nameEndsWith: ".html"
    }, {
        args: ["pylsp"],
        nameEndsWith: ".python"
...
wss.on('connection', ws => {
    ...
    ws.on('message', message => {
        ...
        const writer = servers.find(server => parsed.params.textDocument.uri.endsWith(server.nameEndsWith)).writer;
        if (writer) {
            writer.write(parsed);
        }
    });
